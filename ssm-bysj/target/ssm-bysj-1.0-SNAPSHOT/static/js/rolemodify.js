...
$(function(){
	...
	console.log(roleKey.val());
	console.log(oldKey.val()==roleKey.val());
	console.log(oldKey.val()!=roleKey.val());

	roleKey.bind("blur",function(){

		//ajax后台验证--roleKey是否已存在
		$.ajax({
			type:"GET",//请求类型
			...
			success:function(data){//data：返回数据（json对象）
				if(data.roleKey == "exist" && roleKey.val()!=oldKey.val()){//编码已存在，错误提示
					validateTip(roleKey.next(),{"color":"red"},imgNo+ " 该编码已存在",false);
				}else if(data.roleKey == "error"){//编码输入为空
					validateTip(roleKey.next(),{"color":"red"},imgNo + " 编码不能为空",false);
				}else if (data.roleKey == "true"){//编码可用，正确提示
					validateTip(roleKey.next(),{"color":"green"},imgYes+" 可以使用",true);
				}
			},
			error:function(data){//当访问时候，404，500 等非200的错误状态码
				validateTip(roleKey.next(),{"color":"red"},imgNo+" 您访问的页面不存在",false);
			}
		});
	}).bind("focus",function(){
		//显示友情提示
	 	if(roleKey.val()==oldKey.val()){
			validateTip(roleKey.next(),{"color":"green"},imgYes+" 编码未修改",true);
		}else {
		validateTip(roleKey.next(),{"color":"#666666"},"* 请输入角色编码",false);
		}
	}).focus();
